...
public class HomeServlet extends HttpServlet {
	...
	public void main(HttpServletRequest request, HttpServletResponse response)
		...
		String sql=null;
		if(cid==null){
			sql = "select b.id as id,title,content,created_time,name as category,c.id as categoryid  from blog b,category c where  category_id=c.id order by b.id desc  limit 0,5";
		}else{
			sql = "select b.id as id,title,content,created_time,name as category,c.id as categoryid  from blog b,category c where  category_id=c.id and category_id="+cid+" order by b.id desc  limit 0,5";
		}
		...
		// 查询最新的评论
		sql = "select id,username,content,blog_id as blogid from comment order by id desc limit 0,5";
		List comments = null;
